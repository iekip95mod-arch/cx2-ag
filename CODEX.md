...
Each executor and reviewer identity maintains one role-specific progress comment on its current issue or PR. Assignment records queued work, model startup records actual execution, and the final update records success, failure or cancellation with the configured model, effort and workflow attempt. Later attempts edit the same comment. Reviewer progress is an issue comment and never a formal review, so only the separate native verdict can satisfy the approval gate. Feedback stays inactive until its trusted implementation is present on main.

...
The default Giac root is khi-src. The build queries Makefile.ki rather than duplicating its object list. Numeric dependency scripts are deps/build-gmp.sh and deps/build-mpfr-mpfi.sh. Host and device dependency discovery differ. Read the configured roots before changing include or link settings.

...
`source`: nps/CMakeLists.txt:1318 guards it with `if(LUAJIT_EXECUTABLE AND LUAJIT_FOUND)`. The other two
targets that compile lua_module.cc, `nps_split_module` at line 711 and `nps_nspire_module` at line 917,

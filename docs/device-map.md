...
`optics`, `unit_conversion` (lua_module.cc:4076), `vector_addition` (lua_module.cc:4079),
`vector_cross` (lua_module.cc:4080), `components_to_magnitude_angle`, `magnitude_angle_to_components`,
...
`source`: read on 2026-09-26. nps/lua/nps_v4.lua:2752 sends projectile true for the thrown ball, and
the problem table at nps/lua/nps_v4.lua:2763-2770 carries no projectile key at all, which is how the
...
**`D2Editor` rich text**, the typeset path. mathBox builds it at nps/lua/nps_v4.lua:3180, measureMath
sets the expression at :3214, and the history editor sets its expression at :1438.

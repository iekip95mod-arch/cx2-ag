...
`optics`, `unit_conversion` (lua_module.cc:4146), `gravitation`, `oscillation` and `wave`
(lua_module.cc:4149 to 4151), `pressure`, `hydrostatic`, `buoyancy`, `continuity`, `sensible_heat`,
`latent_heat` and `ideal_gas` (lua_module.cc:4152 to 4158), `vector_addition` (lua_module.cc:4159),
`vector_cross` (lua_module.cc:4160), `components_to_magnitude_angle`, `magnitude_angle_to_components`,
...
`relation_into` at lua_module.cc:3224, which reads the variable names against the model's own term
names (lua_module.cc:3210). Any engine built on `RelationModel` can be exposed the same way with a
one-line binding.
...
`source`: read on 2026-09-26, nps/lua/nps_v4.lua's guided physics browser (`PHYSICS_FIXTURES`) names
`buoyancy`, `catch_up`, `continuity`, `density`, `forces`, `gravitation`, `hydrostatic`, `ideal_gas`,
`kinematics`, `latent_heat`, `magnitude_angle_to_components`, `optics`, `oscillation`,
`planar_kinematics`, `pressure`, `relative_motion`, `sensible_heat`, `unit_conversion`,
`vector_addition`, `vector_cross`, `wave` and `work`.
...
`source`: read on 2026-09-26. nps/lua/nps_v4.lua:2757 sends projectile true for the thrown ball, and
the problem table at nps/lua/nps_v4.lua:2768-2775 carries no projectile key at all, which is how the
...
nps/lua/nps_v4.lua:76-77, so a build missing either one refuses to start rather than offering a
...
manifestCompatibility refuses it as malformed at nps/lua/nps_v4.lua:116 and every StepCAS surface stays
...
**`D2Editor` rich text**, the typeset path. mathBox builds it at nps/lua/nps_v4.lua:3282, measureMath
sets the expression at :3316, and the history editor sets its expression at :1448.
...
`source`: nps/CMakeLists.txt:1335 guards it with `if(LUAJIT_EXECUTABLE AND LUAJIT_FOUND)`. The other two
targets that compile lua_module.cc, `nps_split_module` at line 726 and `nps_nspire_module` at line 932,

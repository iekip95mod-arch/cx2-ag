...
if hasGiac and CHAN_PROBE then
	for _, ext in ipairs({"", ".tns"}) do
		local out = "/documents/ndless/chan-out" .. ext
		local inp = "/documents/ndless/chan-in.txt" .. ext
		chanTry("write" .. ext,
	...
	for _, ext in ipairs({"", ".tns"}) do
		pcall(luagiac.caseval,
			'f:=fopen("/documents/ndless/chan-probe' .. ext .. '");' ..
	...
	if inited then
		...
		if dispinfos then
			...
			if hasGiac then
				...
				gc:setColorRGB(0)
				gc:drawString("Type * to enter shell", 2, scrHeight/2-20, "top")
			else
...
menu = {
       ...
       { "Plots",
       	 { "plot(f(x),x=a..b)",	function() menustring( "plot(" ) end },
       	 { "plotarea(f(x),x=a..b,[method])",	function() menustring( "plotarea(" ) end },
       	 { "plotparam([x(t),y(t)],t=a..b)",	function() menustring( "plotparam(" ) end },
       	 { "plotpolar([r(theta)],theta=a..b)",	function() menustring( "plotpolar(" ) end },
       	 { "plotfield(f(t,y),[t=t1..t2,y=y1..y2])",	function() menustring( "plotfield(" ) end },
       	 { "plotode(f(t,y),[t=t1..t2,y],[t0,y0])",	function() menustring( "plotode(" ) end },
       	 { "plotdensity(f(x,y),[x=x1..x2,y=y1..y2])",	function() menustring( "plotdensity(" ) end },
       	 { "plotcontour(f(x,y),[x=x1..x2,y=y1..y2])",	function() menustring( "plotcontour(" ) end },
       	 { "plotimplicit(f(x,y),[x=x1..x2,y=y1..y2])",	function() menustring( "plotimplicit(" ) end },
	 "-",
       	 { "bar_plot(data_list)",	function() menustring( "bar_plot(" ) end },
       	 { "histogram(data,xmin,size)",	function() menustring( "histogram(" ) end },
       	 { "scatterplot(Xlist,Ylist)",	function() menustring( "scatterplot(" ) end },
       	 { "linear_regression_plot(Xlist,Ylist)",	function() menustring( "linear_regression_plot(" ) end },
       },
       { "Lists",

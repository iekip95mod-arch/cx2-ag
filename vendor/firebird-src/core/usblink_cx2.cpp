...
static bool stream_ack_pending;
static uint16_t pending_stream_sequence;
static struct {
    bool valid;
    uint16_t sequence;
} accepted_stream[256][2];
...
static void handlePacket(const NNSEMessage *message, const uint8_t **streamdata = nullptr, int *streamsize = nullptr)
{
    ...
    if(message->service & AckFlag)
    {
        ...
        if(message->service == (StreamService | AckFlag)
           && message->src == AddrCalc && message->dest == AddrMe
           && stream_ack_pending && message->seqno == pending_stream_sequence)
        {
            stream_ack_pending = false;
    ...
    auto &accepted = accepted_stream[message->src][message->dest == AddrAll];
    if(message->service == StreamService && (message->reqAck & 8)
       && accepted.valid && accepted.sequence == message->seqno)
    {
        return;
    }
...
*/
    ...
    {
        accepted.valid = true;
        accepted.sequence = message->seqno;
        if(streamdata)
...
bool usblink_cx2_send_navnet(const uint8_t *data, uint16_t size)
{
    ...
    *msg = {};
    ...
    stream_ack_pending = true;
    pending_stream_sequence = msg->seqno;
...
void usblink_cx2_reset()
{
    ...
    stream_ack_pending = false;
    memset(accepted_stream, 0, sizeof(accepted_stream));
}

...
uint32_t lcd_read_word(uint32_t addr) {
    ...
    if (offset < 0x200) {
        ...
        uint32_t value;
        memcpy(&value, (uint8_t *)lcd.palette + (offset - 0x200), sizeof value);
        return value;
    } else if (offset < 0x800) {
        return bad_read_word(addr);
    } else if (emulate_cx && offset < 0xC00) {
        uint32_t value;
        memcpy(&value, lcd.cursor_ram + (offset - 0x800), sizeof value);
        return value;
...
write_control:
        ...
        memcpy((uint8_t *)lcd.palette + (offset - 0x200), &value, sizeof value);
        return;
    } else if (offset < 0x800) {
        bad_write_word(addr, value);
        return;
    } else if (emulate_cx && offset < 0xC00) {
        memcpy(lcd.cursor_ram + (offset - 0x800), &value, sizeof value);
        return;
    } else if (emulate_cx && offset < 0xC30) {
        switch (offset) {

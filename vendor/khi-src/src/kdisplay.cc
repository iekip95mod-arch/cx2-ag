...
#define XCAS_ONLY 0x80000000
  ...
  {"volume(P)", 0, "volume d'un polyedre ou d'une sphere", 0, 0, (CAT_CATEGORY_3D )},
				     //{"version", "version()", "Ki 1.5.0, (c) B. Parisse et al. www-fourier.ujf-grenoble.fr/~parisse. License GPL version 2. Interface adaptee d'Eigenmath pour Casio, G. Maia, http://gbl08ma.com", 0, 0, CAT_CATEGORY_PROGCMD},
...
const catalogFunc completeCaten[] = { // list of all functions (including some not in any category)
  ...
  {"from math/... import *", "from math import *", "Access to math or to random functions ([random]) or turtle with English commandnames [turtle]. Math import is not required in Ki", "#from random import *", "#from turtle import *", CAT_CATEGORY_PROG},
  ...
  const char aide_khicas_string[]="Aide Ki";
#ifdef NUMWORKS
...
#define EQW_TAILLE 18
  ...
  bool is_ndl_installer(const string & name){
    const char prefix[]="ndl_installer_";
    return name.compare(0,sizeof(prefix)-1,prefix)==0;
  }

  void nspire_copy_data(int nkeys,BYTE hash[][SHA256_BLOCK_SIZE],GIAC_CONTEXT){
    const char * dirname="/documents/ndl";
    const char * targetdirname="/exammode/usr/ndl";
    DIR *dp;
	...
	continue;
      if ( (s=="khicas.tns" || s=="luagiac.luax.tns" || s=="khicaslua.tns" || s=="ptt.tns" || is_ndl_installer(s) || s=="ndl_resources.tns" || s=="ndl.cfg.tns" || s=="upsilon.tns" || s=="xcasnws.tns")){ // shakeys.tns is not copied, it is retrieved from non-exam mode
	string ss=dirname+("/"+s);
  ...
  DIR * nspire_clear_data(const char * dirname,int nkeys,BYTE hash[][SHA256_BLOCK_SIZE],GIAC_CONTEXT){
    ...
    bool ndl=strcmp(dirname,"/exammode/usr/ndl")==0;
    DIR *dp;
    struct dirent *ep;
    dp = opendir (dirname);
    if (!dp)
	...
	if (ptr && s!="/exammode/usr/ndl" && s!="/exammode/usr/Press-to-Test")
	  res=rmdir(s.c_str());
	// else *logptr(contextptr) << s << '\n'; //
      }
      else {
	if (toplevel || ndl){
	  if (ndl && s=="shakeys.tns")
	    continue;
	...
	if (ndl){
	  if ((is_ndl_installer(s) || s=="ndl_resources.tns" || s=="ndl.cfg.tns") ){
	    string ss=dirname+("/"+s);
	    if (sha_check(ss.c_str(),nkeys,hash))
	      continue;
	  }
  ...
  void nspire_clear_data(GIAC_CONTEXT,bool copy){
    ...
    int nkeys=rsa_check("/exammode/usr/ndl/shakeys.tns",maxkeys,hash);
    if (nkeys<=0)
      nkeys=rsa_check("/documents/ndl/shakeys.tns",maxkeys,hash);
    if (lang==1)
...
#include "kled.cc"
  ...
  void set_exam_mode(int i,GIAC_CONTEXT){
    ...
    const char * exec=0;
    if (i==-1)
      exec="/documents/ndl/ptt.tns";
    else
      exec="/exammode/usr/ndl/ptt.tns";
    ...
    // int res=nl_exec("/documents/ndl/ptt.tns",1,filenames);
    unlink(clef);
    if (i!=-1)
      exam_mode=i;
  }
...
// write exam mode mode to flash at offset pos/8
  ...
  // return 0 if invalid, 1 for Ki, 2 for Omega, 3 for Upsilon, 4 for Epsilon<=18.2.3
  ...
  void menu_setup(GIAC_CONTEXT){
	...
	if (smallmenu.selection==11 && osok==-1){
	  confirm(lang==1?"Activez une fois le mode examen TI":"Activate one time TI exam mode",lang==1?"pour utiliser ensuite celui de Ki":"to enable Ki exam mode");
	  continue;
	}
	if (smallmenu.selection==11 && osok==0){
	  confirm(lang==1?"Ce modele n'est pas compatible":"This model is not compatible",lang==1?"avec le mode examen de Ki":"with Ki exam mode");
	  continue;
	}
	if (smallmenu.selection == 11 && osok>0){
#if defined NSPIRE_NEWLIB && !defined BW
	  if (nspire_exam_mode==1
	    ...
	    *logptr(contextptr) << (lang==1?"Quittez Ki (doc doc) pour rebooter le mode examen\n":"Leave Ki (doc doc) to reboot in exam mode.\n");
	    break;
	  }
	  else {
	    if (osok>0 ||
		...
		*logptr(contextptr) << (lang==1?"Patientez 30 secondes environ\n":"Please wait about 30 secondes\n");
		set_exam_mode(-1,contextptr); // end up with reset()
	      ...
	      confirm((lang==1)?"Desole. Le mode examen de Ki":"Sorry, Ki exam mode",(lang==1)?"ne fonctionne pas sur Nspire CX II":"is not supported on Nspire CX II");
	      break;
	    }
	    if (1
		|| is_cx2
	      ...
	      text.editable=false;
	      text.clipline=-1;
	      text.title = lang==1?"Ki et mode examen":"Ki and exam mode";
	      add(&text,(lang==1)?
		  "Attention, verifiez que le calcul formel est autorise avant d'utiliser Ki en mode examen. En France, c'est en principe autorise lorsque la calculatrice graphique l'est (par exemple au bac)":
		  "Warning! Check that CAS is allowed before running Ki in exam mode.");
	      const char exam_mode_fr_string[]="Pour utiliser Ki en mode examen, il faut effectuer une preparation chez soi quelques heures avant avec une connection PC ou quelques minutes avant l'examen avec un autre etudiant ayant une Nspire CX ou CX II.\nLancer le mode examen sur la calculatrice cible (esc-on), recopier ndl et khicas.tns (ou luagiac.luax.tns et khicaslua.tns) sur la calculatrice cible en mode examen. Avec 2 calculatrices, recommencez sur l'autre calculatrice (mettre l'autre calculatrice en mode examen et copiez dessus ndl et khicas).\nActiver ndl (cable debranche) puis lancez Ki puis touche calculatrice (en-dessous de esc) puis selectionner l'item 11. mode examen, valider : ceci va effacer les donnees et desactiver le clignotement des leds.\n\nAu debut de l'examen, lorsque le surveillant demande d'activer le mode examen, quittez Ki en tapant doc doc (ou appuyez sur reset), le mode examen sera a nouveau actif et les leds clignoteront. Vous pouvez activer ndl et lancez Ki.\nPour les institutions n'acceptant pas Ki en mode examen: demandez a vos etudiants de redemarrer la calculatrice, puis faire esc-on et reinitialiser le mode examen.";
	      const char exam_mode_en_string[]="Running Ki in exam mode requires preparation at home with a PC or a few minutes with another student having a Nspire CX/CXII.\nActivate exam mode on the target calculator (esc-on), connect the PC or the other calculator, copy ndl and khicas.tns (or luagiac.luax.tns and khicaslua.tns) to the target calc (kept in exam mode). With 2 calculators, repeat on the other calculator.\n Activate ndl (disconnect the link) and run Ki. Type the calculator key below esc then select 11. Exam mode. This will desactivate leds blinking and clear data. When exam begins, quit Ki (doc doc) or press reset, exam mode will be active again and leds will blink. Activate ndl and run Ki.\n\nFor institutions who do not want to allow Ki, ask your students to reset their calculator, press esc-on and restart exam mode, this will clear ndl and Ki.";
  ...
  */
  ...
  int restore_session(const char * fname,GIAC_CONTEXT){
    ...
    if (!load_console_state_smem(filename.c_str(),contextptr)){
      ...
      Bdisp_AllClr_VRAM();
      int x=0,y=0;
      PrintMini(x,y,"Ki 1.9 (c) 2022 B. Parisse",TEXT_MODE_NORMAL, COLOR_BLACK, COLOR_WHITE);
      y +=18;
...
#endif
      y += 18;
      PrintMini(x,y,((lang==1)?"pour quitter Ki.":"to leave Ki."),TEXT_MODE_NORMAL,COLOR_BLACK, COLOR_WHITE);
      y += 18;
      PrintMini(x,y,(lang==1)?"Si le calcul formel est interdit":"If CAS is forbidden!",TEXT_MODE_NORMAL, _red, COLOR_WHITE);
      y += 18;
#ifdef NSPIRE_NEWLIB
      PrintMini(x,y,(lang==1)?"quittez Ki (menu menu menu)":"Leave Ki (menu menu menu)",TEXT_MODE_NORMAL, _red, COLOR_WHITE);
...
#else
      PrintMini(x,y,(lang==1)?"quittez Ki (HOME HOME HOME)":"Leave Ki (HOME HOME HOME)",TEXT_MODE_NORMAL, _red, COLOR_WHITE);
	...
	*logptr(contextptr) << "et empeche d'utiliser Ki\n";
      } else {
	...
	*logptr(contextptr) << "it's incompatible with Ki\n";
      }
#endif
      Bdisp_AllClr_VRAM();
#ifdef GIAC_SHOWTIME
  ...
  int restore_session(const char * fname,GIAC_CONTEXT){
	...
	lang=0;
...
#endif
      int x=0,y=0;
      PrintMini(x,y,"Ki 1.6 (c) 2020 B. Parisse",TEXT_MODE_NORMAL, COLOR_BLACK, COLOR_WHITE);
      y +=18;
      PrintMini(x,y,"et al, License GPL 2",TEXT_MODE_NORMAL,COLOR_BLACK, COLOR_WHITE);
      y += 18;
#ifdef NSPIRE_NEWLIB
      ...
      y += 18;
#ifdef NSPIRE_NEWLIB
      PrintMini(x,y,(lang==1)?"quittez Ki (doc doc doc)":"Leave Ki (doc doc doc)",TEXT_MODE_NORMAL, _red, COLOR_WHITE);
      if (confirm("Interpreter? enter: Xcas, esc: MicroPython",(lang==1?"Peut se modifier depuis menu configuration":"May be changed later from menu configuration"),false,130)==KEY_CTRL_F6){
	python_compat(4,contextptr);
	xcas_python_eval=1;
	*logptr(contextptr) << "Micropython interpreter\n";
...
#ifdef NSPIRE_NEWLIB
	  // KEY_CTRL_MENU is the DOC key here, k_csdk.c line 1148, so the hint names doc rather than menu.
	  smallmenuitems[16].text = (char*) ((lang==1)?"Quitter (doc doc)":"Quit (doc doc)");
#else
  ...
  */
  ...
  int console_main(GIAC_CONTEXT,const char * sessionname){
    ...
    if ((osid & 0xffff0000)==0x10410000){
      confirm("Ki exammode is incompatible with OS 5.3","Downgrade to 5.2 with backSpire");
    }
#if 0
    if (osok && is_cx2){
      int N=0x800;

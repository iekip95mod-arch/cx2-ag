...
int ext_main(){
  while (1){
    ...
    os_draw_string(0,20,_WHITE,_BLACK,"1. Ki shell");
...
int khicas_addins_menu(GIAC_CONTEXT){
  ...
  smallmenuitems[smallmenu.numitems-1].text = (char*)((lang==1)?"Quitter Ki":"Leave Ki");
  while(1) {
    int sres = doMenu(&smallmenu);
...
void handle_flash(GIAC_CONTEXT){
  ...
  if (buf64k==0){
    confirm(lang==1?"Pas assez de memoire RAM.":"RAM Memory full",lang==1?"Purgez et relancez Ki":"Purge and restart Ki");    
    return;
  }
#ifndef NUMWORKS
  char * freeptr=0;

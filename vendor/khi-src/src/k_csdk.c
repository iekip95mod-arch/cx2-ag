// implementation of the minimal C SDK for Ki
int (*shutdown)()=0;
...
int getkey(int allow_suspend){
  ...
  for (;;){
	...
	is_cx2 && nspire_ctrl && on_key_pressed()){
      ...
      nspire_draw_string(60,120,0,0xffff,Regular12,"Quit Ki to shutdown",false);
      nspire_ctrl=false;
      statusline(1);
      continue;
    }
	...
	*(volatile unsigned *) (NSPIRE_CONTRAST_ADDR+4)=0xffff;
	...
	if (!exam_mode && nspire_exam_mode!=2 && shutdown
	  ...
	  if (curtime-offtime>7200){
	    ...
	    *(volatile unsigned *)NSPIRE_CONTRAST_ADDR=oldval;
	    statuslinemsg("Press ON to disable Ki auto shutdown");
	...
	nspire_draw_string(25,60,0,0xffff,Regular12,"Mode examen de Ki, avec CAS.",false);
	...
	nspire_draw_string(25,150,0,0xffff,Regular12,"Quitter Ki (ou appuyer sur reset)",false);

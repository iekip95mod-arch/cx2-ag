...
static unsigned const ploader_hook_addrs[NDL_MAX_OSID+1] =
...
static unsigned const end_of_init_addrs[NDL_MAX_OSID+1] =
...
// OS-specific
// get_res_string + 0xC8
static unsigned const error_msg_patch_addrs[NDL_MAX_OSID+1] =
...
// close_document
static unsigned const close_document_addrs[NDL_MAX_OSID+1] =
					       {0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
						0x0, 0x0, 0x0, 0x0,
						0x0, 0x0, 0x0, 0x0,
						0x0, 0x0, 0x0, 0x0,
...
/* argv[0]=
 *         NULL if loaded by ndl's stage1 at installation or OS startup
 *         "L" if loaded by a third party loader such as nLaunchy
 *         <path to ndl_resources> if run from the OS documents screen for uninstallation      
 */
int main(int __attribute__((unused)) argc, char* argv[]) {
	...
	if (!installed) {
		...
		else // 3.9, 4.0.3, 4.2.0, 4.4.0 and 4.5.0
		{
			...
			if (argv[0] && argv[0][0] == 'P') {
				int persistence_status = persistency_install();
				if (persistence_status == 1)
					show_msgbox("ndl persistence", "Persistent setup did not complete. ndl is loaded for this session. Check storage and retain any .ndl recovery files before trying again.");
				else if (persistence_status == 3)
					show_msgbox("ndl persistence", "Persistence is installed. Backup cleanup did not complete. Retain the .ndl recovery files and check storage before trying again.");
			}
		...
		// Cache the path to ndl.cfg.tns to keep working even if the virtual root changes
		cfg_open();
		cfg_close();
	...
	if (installed) { // ndl_resources.tns run: uninstall
		if (show_msgbox_2b("ndl", "Do you really want to uninstall ndl r" STRINGIFY(NDL_REVISION) "?\nThe device will reboot.", "Yes", "No") == 2)
			return 0;

		if (persistency_uninstall()) {
			show_msgbox("ndl uninstall", "Persistence could not be disabled. ndl remains installed. Check storage before trying again.");
			return 0;
		}
		ins_uninstall();
	}
...
// OS-specific
// gui_gc_drawIcon + 4
const unsigned ins_successmsg_hook_addrs[NDL_MAX_OSID+1] =
					{0x0, 0x0, 0x0, 0x0, 0x0, 0x0,
...
const unsigned ins_successmsg_icon[NDL_MAX_OSID+1] =
...
HOOK_DEFINE(ins_successsuccessmsg_hook) {
	...
	if (HOOK_SAVED_REGS(ins_successsuccessmsg_hook)[2] == ins_successmsg_icon[ut_os_version_index]) {
		// The banner this used to draw is gone. Everything else in this hook stays: it closes the
		// resources document, and it counts frames so it can uninstall itself and clear the cache.
		// Not installing the hook at all would lose those.
		static int i = 6;
		if (!i--) {

...
// Marker at the beginning of a plain-old ndl program. Deprecated, only used for ndl itself, Zehn is now used for programs.
#define PRGMSIG "PRG"
...
struct ld_diagnostic {
	const char *stage;
	int code;
	size_t bytes;
};
static inline int ld_diagnostic_set(struct ld_diagnostic *diagnostic, const char *stage, int code, size_t bytes) {
	if (diagnostic) {
...
int ld_exec_with_args_diagnostic(const char *path, int argc, char *argv[], void **resident_ptr, struct ld_diagnostic *diagnostic);
...
/* N-ext is a convention for TI-Nspire extensions such as ndl.
...
extern unsigned int syscall_addrs[NDL_MAX_OSID+1][__SYSCALLS_LAST+1];
...
/* persistency.c */
// Install: 0 complete, 1 failed, 2 skipped, 3 complete with a cleanup warning.

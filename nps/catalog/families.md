...
family_envelope_version 1
accepted_expression_grammar sums, products, negations and quotients of integers and one variable, whole and rational powers with least common exponent denominator at most 12 and every exponent numerator at most 1024 in size, and a total degree low enough that the exact check values fit in 20000 bits, measured as x^851*x rewritten and x^852*x refused, with x^(851/12)*x^(1/12) and sqrt(x)^851*sqrt(x) at the same boundary, square roots and absolute values, where every rational power and every square root is taken of a single term
...
unsupported_near_neighbors two or more variables, roots of sums, functions other than sqrt and abs, decimals, exponent denominators whose least common multiple exceeds 12, exponent numerators above 1024 in size, total degree past the 20000-bit value limit of the check, which is refused as outside the envelope with the reason that the check cannot compute it exactly, and numbers under a root with a factor past the trial division bound of 100000

...
- check.yml runs fast first, then full and emulator in parallel once fast succeeds. They used to wait for the current PR review as well, and that line went back in the moment an approving review is required to merge again, because otherwise a branch merges before the suites report and they prove nothing. The linux-parity, device and codeql jobs have all been removed. The emulator job boots TI OS and uploads the frame, and you can drive the same harness yourself, so read [Drive the emulator yourself](#drive-the-emulator-yourself).

...
Executors and reviewers each maintain one progress comment per assigned identity and role. It shows queued, running, publishing and terminal state together with the configured model, effort and workflow attempt. Later runs edit that comment, including failure or cancellation, so an old start message does not remain current. Report an alias as an alias, never as a verified model version. Formal reviewer findings and verdicts remain native reviews, separate from the progress comment.

...
export async function reviewHistory({ repository, pr, head, base }, api) {
  ...
  const collect = async (kind, limit = 10) => {
    const entries = [];
    for (let page = 1; page <= limit; page++) {
      const batch = await api(`${endpoint}/${kind}?per_page=100&page=${page}`);

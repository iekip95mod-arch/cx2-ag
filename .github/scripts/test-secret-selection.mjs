  ...
  for (const [workflow, expected] of Object.entries(pools)) {
    ...
    for (let index = 0; index < expected.length; index++) {
      ...
      for (const name of [...pool.map(bot => bot.secretName), 'UNKNOWN_SECRET']) {
        const translated = expression.replace(/(?:needs|steps)\.[a-z_-]+\.outputs\.secret_name|matrix\.secret_name/g, JSON.stringify(name)).replace(/secrets\.([A-Z0-9_]+)/g, (_, key) => JSON.stringify(`value:${key}`));

...
export function geminiSettings({ mode, home, workspace, temporary }) {
  ...
  return { enableTerminalSandbox: mode !== 'reply', toolPermission: 'proceed-in-sandbox', artifactReviewPolicy: 'always-proceed', permissions: { allow, deny } };
}

export function prepareGemini({ credentials, mode, home, workspace, temporary }) {
  if (!credentials?.trim()) throw Error('ANTIGRAVITY_OAUTH_CREDS is required. No subscription login is configured');
  JSON.parse(credentials);
  const settings = geminiSettings({ mode, home, workspace, temporary });
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  try {
    prepareGemini({ credentials: process.env.OAUTH_CREDS, mode: process.env.GEMINI_MODE, home: process.env.HOME, workspace: process.env.GITHUB_WORKSPACE, temporary: process.env.RUNNER_TEMP });

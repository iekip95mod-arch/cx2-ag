...
test('the actual entry point posts with configured metadata without printing its credential', () => {
  ...
  const env = { PATH: `${bin}:${dirname(process.execPath)}:${process.env.PATH}`, GH_TOKEN: 'executor-start-fixture-token', GITHUB_EVENT_PATH: event, GITHUB_RUN_ID: '123', GITHUB_RUN_ATTEMPT: '1', WORKER_MODEL: 'opus', WORKER_EFFORT: 'high', FEEDBACK_FIXTURE: config };
